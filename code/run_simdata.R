                      ...
                      .combine = bind_rows) %do% {
                        ...
                        df0 <- foreach(j = 1:n_rep,
                                       .combine = bind_rows) %do% {
                                         ...
                                         b <- foreach(i = 1:length(list_est),
                                                      .combine = bind_rows) %do% {
                                           b <- summary(list_est[[i]], "fixed")[2, "Estimate"]
                                           se_b <- summary(list_est[[i]], "fixed")[2, "Std. Error"]
                                           return(tibble(b = b, se_b = se_b))
                                         }
...
df_power <- df_simdata %>% 
  group_by(group) %>% 
  do(y = coef(lm(log(-z) ~ log(p), data = .))[2]) %>% 
  mutate(y = unlist(y))

# export ------------------------------------------------------------------

saveRDS(df_simdata,
        file = here::here("output/data_simulate.rds"))

saveRDS(df_power,
        file = here::here("output/data_power.rds"))
