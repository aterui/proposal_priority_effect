...
First, \ul{\emph{our proposal is conceptually innovative}} because our new method has the potential to provide an unforeseen answer to ``when do priority effects occur?''
...
With this sensitivity criterion, we performed a preliminary analysis of the method's performance.
We used the model formula presented in Equation \ref{eq:ricker} (a Ricker model) to produce simulated time series data.
We considered XXX parameter combinations, within each of which 5 replicates of time series data were produced.
Parameter values were chosen to yield a sufficient variation in the leading eigenvalue $|\lambda_{max}|$ (Table \ref{tab:param1}).
...
A Beverton-Holt (BH) model is another model widely used in ecology, describing the temporal dynamics of competitive communities:
...
Hence, as in a Ricker model, a BH model can be simplified as follows:
...
Note that we used a Taylor expansion to derive this first-order approximation.
...
\begin{table}
    ...
    \begin{tabular}{llll}
         ...
         \hline
         Fukami and Morin 2003 \citep{fukami_productivity-biodiversity_2003} & Protist & add later & 18\\
         Fukami 2004 \citep{fukami_assembly_2004} & Protist & add later & 14\\
         Price and Morin 2004 \citep{price_colonization_2004} & Protist & $\ge 20$ & $2$ or $3$ \\
         Zhang and Zhang 2007 \citep{zhang_colonization_2007} & Algae & $13$ & $2$\\
         Jiang and Patel 2008 \citep{jiang_community_2008} & Protist & add later & $10$\\
         Tucker and Fukami 2014 \citep{tucker_environmental_2014} & Bacteria and yeasts & $9$ & $4$\\
         Pu and Jiang 2015 \citep{pu_dispersal_2015} & Protist & add later & $10$\\
         Ojima and Jiang 2017 \citep{ojima_interactive_2017} & Protist & $23$ & $5$\\
         Hsu and Moeller 2021 \citep{hsu_metabolic_2021} & Protist & $13~\mbox{to}~22$ & $2$\\
         Ojima et al. 2022 \citep{ojima_priority_2022} & Bacteria & $7$ & $2$ or $4$\\
         \hline
    \end{tabular}
    \label{tab:expdata}
\end{table}
...
We applied our method to the dataset in Hsu and Moeller 2021 \citep{hsu_metabolic_2021}, serving as a preliminary analysis of real biological data.
